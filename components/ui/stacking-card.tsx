...
export const Card = ({
  ...
  return (
    <div
      ref={container}
      className='flex sticky top-0 justify-center items-center h-screen'
    >
      <motion.div
        style={{
      ...
      >
        ...
        <div className={`flex gap-10 mt-5 h-full`}>
          ...
          >
            ...
            >
              <img src={url} alt='image' className='object-cover absolute inset-0 w-full h-full' />
            </motion.div>
          </div>
        </div>
      </motion.div>
...
interface ComponentRootProps {
  ...
  isolated?: boolean; // 是否隔离模式（不使用 ReactLenis，避免与 GSAP 冲突）
}
...
const Component = forwardRef<HTMLElement, ComponentRootProps>(({ projects, isolated = false }, ref) => {
  ...
  const content = (
    <div className='bg-black' ref={container}>
      <section className='w-full text-white bg-slate-950' style={{ minHeight: isolated ? `${projects.length * 100}vh` : undefined }}>
